...
class Settings(BaseSettings):
    ...
    database_user_uri: str = f"mysql+pymysql://{user}:{userpassword}@{host}:{port}/{database}"
    database_maria_uri: str = f"mysql+pymysql://{adminuser}:{adminpassword}@{host}:{port}/{database}"
    app: object={}
    engine: object={}
