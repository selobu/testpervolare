...
class User(SQLModel, table=True):
    ...
    nombre_completo: str = Field(sa_column=Column(String(400)), unique=True, description="User fullname")
...
class UserRegister(SQLModel):
    ...
    municipio: str = Field(sa_column=Column(String(200)), description="State")
    direccion: str = Field(sa_column=Column(String(250)), description="Address")
    email: Optional[EmailStr] = Field(default=None, primary_key=True, description="User email")
    password: str = Field(sa_column=Column(String(300)), nullable=False, description="User password only allowed a-z A-Z 0-9 and */+-$%&", max_length=12, min_length=8)